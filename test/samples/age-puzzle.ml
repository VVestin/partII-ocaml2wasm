...
let rec find answer =
  match answer with (min_son_age, max_grandpa_age) ->
    match (min_son_age >= max_grandpa_age) with
      true -> (-1, -1)
    | false ->
        let rec find' answer = match answer with (son_age, grandpa_age) ->
          match son_age = grandpa_age with
            true -> (-1, -1)
          | false ->
              match is_valid_answer (son_age, grandpa_age) with
                true -> (son_age, grandpa_age)
              | false -> find' (son_age + 1, grandpa_age) in
